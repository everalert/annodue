...
const CoreSettings = @import("../util/core/core_settings.zig");
...
pub fn init(arena_perm: Allocator, _: Allocator) !void {
    ...
    try CoreSettings.ASettings.init(memory);
}

pub fn deinit() !void {
    try CoreSettings.ASettings.saveAuto();
    CoreSettings.ASettings.deinit();
}
...
pub fn ASectionOccupy(
    section: Handle,
    name: [*:0]const u8,
    fnOnChange: ?*const fn ([*]CoreSettings.ASettingSent, usize) callconv(.C) void,
) callconv(.C) Handle {
    return CoreSettings.ASettings.sectionOccupy(
        WorkingOwner(),
...
pub fn ASettingOccupy(
    section: Handle,
    name: [*:0]const u8,
    value_type: CoreSettings.Setting.Type,
    value_default: CoreSettings.ASettingSent.Value,
    value_ptr: ?*anyopaque,
    fnOnChange: ?*const fn (CoreSettings.ASettingSent.Value) callconv(.C) void,
) callconv(.C) Handle {
    if (!WorkingOwnerIsSystem() and section.isNull()) return NullHandle;
    return CoreSettings.ASettings.settingOccupy(
        WorkingOwner(),
        if (section.isNull()) null else section,
        name,
        value_type,
...
pub fn ASettingVacate(handle: Handle) callconv(.C) void {
    CoreSettings.ASettings.settingVacate(handle);
}
...
pub fn ASettingUpdate(handle: Handle, value: CoreSettings.ASettingSent.Value) callconv(.C) void {
    CoreSettings.ASettings.settingUpdate(handle, value);
}
...
pub fn AVacateAll() callconv(.C) void {
    ...
    CoreSettings.ASettings.vacateOwner(WorkingOwner());
}
...
pub fn ASettingResetAllDefault() callconv(.C) void {
    if (!WorkingOwnerIsSystem()) return;
    CoreSettings.ASettings.settingResetAllToDefaults();
}
...
pub fn ASettingResetAllFile() callconv(.C) void {
    if (!WorkingOwnerIsSystem()) return;
    CoreSettings.ASettings.settingResetAllToSaved();
}
...
pub fn ASettingCleanAll() callconv(.C) void {
    if (!WorkingOwnerIsSystem()) return;
    CoreSettings.ASettings.settingRemoveAllVacant();
}
...
pub fn ASave() callconv(.C) void {
    if (!WorkingOwnerIsSystem()) return;
    CoreSettings.ASettings.save() catch {};
}
...
pub fn ASaveAuto() callconv(.C) void {
    CoreSettings.ASettings.saveAuto() catch {};
}
...
pub fn OnPluginInitA(owner: u16) callconv(.C) void {
    CoreSettings.ASettings.sectionRunUpdateOwner(owner);
}

pub fn OnPluginDeinitA(owner: u16) callconv(.C) void {
    CoreSettings.ASettings.vacateOwner(owner);
}
...
pub fn GameLoopB(gf: *GlobalFn) callconv(.C) void {
    ...
    CoreSettings.ASettings.hotReloadUpdate(rti.TIMESTAMP.*);
}
...
fn drawSettings(gf: *GlobalFn, section: ?Handle, x_ref: *i16, y_ref: *i16) void {
    for (CoreSettings.ASettings.data_settings.values.items) |value| {
    ...
    for (0..CoreSettings.ASettings.data_sections.values.items.len) |i| {
        const value: *CoreSettings.Section = &CoreSettings.ASettings.data_sections.values.items[i];
        if ((section == null) != (value.section == null)) continue;
        if (section != null and
            (value.section.?.generation != section.?.generation or
            value.section.?.index != section.?.index)) continue;
        ...
        x_ref.* += 12;
        const handle: Handle = CoreSettings.ASettings.data_sections.handles.items[i];
...
// NOTE: use in testing
fn testUpdateSet1(_: CoreSettings.ASettingSent.Value) callconv(.C) void {
